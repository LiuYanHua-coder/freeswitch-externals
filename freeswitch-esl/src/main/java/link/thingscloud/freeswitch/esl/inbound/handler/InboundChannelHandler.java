...
public class InboundChannelHandler extends SimpleChannelInboundHandler<EslMessage> {
    ...
    public void channelActive(ChannelHandlerContext ctx) throws Exception {
        ...
        log.info("channelActive remoteAddr : {}", remoteAddr);
    ...
    public void userEventTriggered(ChannelHandlerContext ctx, Object evt) throws Exception {
        if (evt instanceof IdleStateEvent && ((IdleStateEvent) evt).state() == IdleState.READER_IDLE) {
            log.info("userEventTriggered remoteAddr : {}, evt state : {} ", remoteAddr, ((IdleStateEvent) evt).state());
    ...
    public EslMessage sendSyncSingleLineCommand(final String command) {
        if (isTraceEnabled) {
            log.info("sendSyncSingleLineCommand command : {}", command);
        }
    ...
    public EslMessage sendSyncMultiLineCommand(final List<String> commandLines) {
        ...
        sb.append(LINE_TERMINATOR);
        if (isTraceEnabled) {
            log.info("sendSyncMultiLineCommand command : {}", sb.toString());
        }
        syncLock.lock();
        try {
            syncCallbacks.add(callback);
    ...
    public String sendAsyncCommand(final String command) {
        ...
        if (isTraceEnabled) {
            log.info("sendAsyncCommand command : {}, response : {}", command, response);
        }
    ...
    static class SyncCallback {
        ...
        void handle(EslMessage response) {
            ...
            log.info("releasing latch for response [{}]", response);
            latch.countDown();
        }
    }
